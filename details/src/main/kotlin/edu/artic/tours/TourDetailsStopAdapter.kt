...
import edu.artic.details.databinding.CellTourDetailsStopBinding
...
class TourDetailsStopAdapter : AutoHolderRecyclerViewAdapter<TourDetailsStopCellViewModel>() {
    ...
    override fun View.onBindView(
        ...
        with(holder.binding as CellTourDetailsStopBinding) {
            item.titleText.bindToMain(tourStopTitle.text()).disposedBy(item.viewDisposeBag)
            item.galleryText.bindToMain(tourStopGallery.text()).disposedBy(item.viewDisposeBag)
            ...
            tourNumber.text = context.getString(R.string.tour_stop_position, position + 1)

            item.imageUrl.observeOn(AndroidSchedulers.mainThread()).subscribe {
                GlideApp.with(holder.binding.root.context).load(it)
